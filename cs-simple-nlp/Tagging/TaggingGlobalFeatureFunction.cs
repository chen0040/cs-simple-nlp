...
{
    public class TaggingGlobalFeatureFunction : IFeatureFunction<SentenceStructure, TagSequence, int>
    {
        ...
        public TaggingGlobalFeatureFunction(YDomainEntity yDomainEntity)
        {
        ...
        public void GenerateFeatures(IList<PairedDataInstance<SentenceStructure, TagSequence>> training_data)
        {
            Reset();

            Dictionary<string, int> histogram = new Dictionary<string, int>();
            foreach (PairedDataInstance<SentenceStructure, TagSequence>sentence_rec in training_data)
            {
                int sentence_length = sentence_rec.Entry.Count;
                for (int i = 0; i < sentence_length; ++i)
                {
                    HistoryInstance history = sentence_rec.Entry[i];
                    string tag_i = sentence_rec.Label[i];
                    string w_i = history.w_i;
            ...
            foreach (PairedDataInstance<SentenceStructure, TagSequence> sentence_rec in training_data)
            {
                ...
                for (int i = 0; i < sentence_length; ++i)
                {
                    HistoryInstance rec = sentence_rec.Entry[i];
                    ExtractFeatures(rec, sentence_rec.Label[i], feature_set);
                }
        ...
        public List<int> Get(SentenceStructure x, TagSequence y)
        {
